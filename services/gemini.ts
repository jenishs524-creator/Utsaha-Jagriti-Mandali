...
export const generateDevotional = async (topic: string): Promise<string> => {
  try {
    const response = await ai.models.generateContent({
      ...
      Tone: Warm, pastoral, uplifting, suitable for the congregation of Utsaha Jagriti Mandali based in Kathmandu, Nepal.`,
      config: {
        systemInstruction: "You are a helpful and wise pastoral assistant for Utsaha Jagriti Mandali.",
        temperature: 0.7,
      }
    });
